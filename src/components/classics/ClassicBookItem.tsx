...
export default function ClassicBookItem({
  ...
  return (
    <div className="h-full rounded-lg p-0.5 font-sans">
      <button
        type="button"
        onClick={openClassic}
        disabled={loading}
        ...
        className="group/cover block w-full cursor-pointer rounded-xl text-left outline-none transition hover:opacity-[0.98] focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-wait"
      >
        <div className="relative aspect-[2/3] w-full shrink-0 overflow-hidden rounded-xl shadow-sm ring-1 ring-black/10 dark:from-neutral-800 dark:to-neutral-900 dark:ring-white/10">
          {coverSrc ? (
            <img
              src={coverSrc}
              alt=""
        ...
        <h3 className="ui-trending-title mt-0 block rounded-sm pt-1.5 leading-snug group-hover/cover:underline">
          {book.title}
        </h3>
      </button>
      {error ? (
